...
<script>
import { getDetail, getProComments } from '@/api/product'
  ...
  methods: {
    ...
    async addCart () {
      if (!this.$store.getters.token) {
        this.$dialog.confirm({ confirmButtonText: '去登录', cancelButtonText: '再逛逛', title: '温馨提示', message: '此时需要登录才能继续操作' }).then(() => {
          this.$router.replace({
            path: '/login',
            query: {
      ...
      const skuId = this.detail && this.detail.skuList && this.detail.skuList[0] && this.detail.skuList[0].goods_sku_id
      if (!skuId) {
        this.$toast('获取商品规格失败')
        return
      }
      const { data } = await addCart(this.goodsId, this.addCount, skuId)
